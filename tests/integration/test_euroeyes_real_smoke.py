"""Real EuroEyes end-to-end smoke — Phase 1 final validation.

Gated by ``PTE_SMOKE_HIT_REAL_APIS=true`` so it doesn't run in default
CI. When enabled, reads the real EuroEyes markdown from
``~/data_inputs/euroeyes/`` (3 files: annual_report_2024.md,
interim_h1_2025.md, wacc_inputs.md), runs the full pipeline against
**live** Anthropic + FMP + yfinance APIs, and asserts structural
outcomes — not specific numeric values (those will vary over time and
are pinned in the sprint report).

Estimated cost per run: **$5–$10** LLM + metered API calls.

Checklist the test enforces:

- 3 documents ingested (2 reports + WACC file).
- Section extractor produces ≥ 6 parsed sections (IS, BS, CF, taxes,
  leases, segments).
- Cross-check report has FMP + yfinance data for 1846.HK.
- CanonicalCompanyState persisted with InvestedCapital + NOPATBridge +
  KeyRatios present.
- ValuationSnapshot has 3 scenarios, E[V] > 0.
- Ficha composed and persisted.
"""
...
def _required_files() -> list[Path]:
    return [
        _REAL_INPUTS / "annual_report_2024.md",
        _REAL_INPUTS / "interim_h1_2025.md",
...
def _setup(
    ...
    # --- LLM + market-data providers (real) ---------------------------
...
# ======================================================================
# The smoke — structural assertions only, values will vary year-to-year
# ======================================================================
...
class TestEuroEyesRealSmoke:
    ...
    def test_end_to_end_real_apis(self, _setup: dict[str, object]) -> None:
        ...
        cost_tracker: CostTracker = _setup["cost_tracker"]  # type: ignore[assignment]

        # 1. Documents ingested
        assert len(doc_repo.list_documents("1846.HK")) >= 3, (
            "Expected 3 documents ingested (AR + Interim + WACC)."
        )

        # 2. Run the pipeline (real Anthropic + FMP + yfinance calls)
        outcome = asyncio.run(pipeline.process("1846.HK", wacc_path=wacc_path))

        # 3. Pipeline finished — 10 stages
        ...
        # At least 4 sections parsed (IS, BS, CF, at least one note)
        assert state.reclassified_statements, "No reclassified statements produced."
        ...
        # 5. Cross-check report has both sources
        assert outcome.cross_check_report is not None
        report = outcome.cross_check_report
        assert len(report.metrics) > 0, "Cross-check produced no metrics."
        # At least revenue should be fetchable from both providers
        revenue_metric = next(
            (m for m in report.metrics if m.metric == "revenue"), None
        )
        assert revenue_metric is not None, "Revenue not in cross-check report."

        # 6. Valuation snapshot persisted with 3 scenarios
        ...
        # 8. Cost tracker recorded activity
        assert cost_tracker.ticker_total("1846.HK") > 0
